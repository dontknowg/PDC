...
# OBS: 180 não é um valor padrão do ENEM (0/40/80/120/160/200).
# Mantido pendente de confirmação — remova se não fizer parte da sua rubrica.
...
def _normalizar_telefone(contato) -> str:
    """Retorna o número no formato 55DDDNUMERO (só dígitos) ou '' se inválido.

    Aceita entradas com ou sem código do país e com máscara
    (parênteses, traços, espaços).
    """
    d = "".join(filter(str.isdigit, str(contato or "")))

    # Remove o código do país se já veio junto (12 ou 13 dígitos = 55 + DDD + número)
    if d.startswith("55") and len(d) in (12, 13):
        d = d[2:]

    # Agora esperamos DDD (2) + número (8 ou 9) = 10 ou 11 dígitos
    if len(d) < 10 or len(d) > 11:
        return ""
    return f"55{d}"


def _resolver_jid(host: str, instance_key: str, token: str, telefone_limpo: str):
    """Consulta a MegaAPI e devolve o JID real do número no WhatsApp.

    Resolve o problema do 9º dígito: o número pode estar cadastrado com o 9
    (ex.: 5582999224032), mas registrado no WhatsApp sem ele
    (ex.: 558299224032). A API retorna o JID correto em 'jid'.

    Retorna (jid, existe):
      - (jid_real, True)  -> número existe; use jid_real no envio
      - (None, False)     -> número NÃO tem WhatsApp
      - (None, None)      -> não foi possível checar (segue com o número padrão)
    """
    url = f"https://{host}/rest/instance/isOnWhatsApp/{instance_key}"
    try:
        r = requests.get(
            url,
            params={"jid": f"{telefone_limpo}@s.whatsapp.net"},
            headers={"Authorization": f"Bearer {token}"},
            timeout=15,
        )
        data = r.json()
    except (requests.exceptions.RequestException, ValueError) as e:
        print(f"[chamar_aluno] Não foi possível checar o número ({telefone_limpo}): {e}")
        return None, None

    if not data.get("exists"):
        return None, False
    return data.get("jid") or f"{telefone_limpo}@s.whatsapp.net", True
...
def chamar_aluno(id_aluno: str, nome_aluno: str, contato_aluno: str) -> bool:
    ...
    try:
        ...
        st.error("WhatsApp não está configurado nos secrets (bloco [whatsapp]).")
        return False
    ...
    # 2) Valida o número ANTES de chamar a API
    telefone_limpo = _normalizar_telefone(contato_aluno)
    if not telefone_limpo:
        st.warning(f"Número inválido para {nome_aluno}: '{contato_aluno}'. Corrija o cadastro.")
        return False
    ...
    # 3) Resolve o número REAL no WhatsApp (corrige o 9º dígito).
    #    Se a checagem falhar por rede, cai no número padrão e tenta enviar.
    jid, existe = _resolver_jid(host, instance_key, token, telefone_limpo)
    if existe is False:
        st.warning(
            f"O número de {nome_curto} ({contato_aluno}) não tem WhatsApp ativo. "
            "Confira o cadastro do aluno."
        )
        return False
    destino = jid or f"{telefone_limpo}@s.whatsapp.net"

    url_api = f"https://{host}/rest/sendMessage/{instance_key}/text"
    payload = {"messageData": {"to": destino, "text": mensagem}}
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}

    # 3) Chamada HTTP, tratando falha de conexão separadamente
    try:
        resp = requests.post(url_api, json=payload, headers=headers, timeout=15)
    except requests.exceptions.RequestException as e:
        print(f"[chamar_aluno] Falha de conexão: {e} | url={url_api}")
        st.error(
            "Não foi possível falar com a API do WhatsApp. "
            "Confira o 'host' nos secrets — deve ser só o domínio, "
            "ex.: apinocode01.megaapi.com.br (sem https:// e sem barra no fim)."
        )
        return False

    # 4) Interpreta a resposta com segurança (JSON de verdade, não busca por texto)
    try:
        data = resp.json()
    except ValueError:
        data = {}
    ...
    # 5) Envio OK — marca como chamado. Se essa etapa falhar (ex.: coluna
    #    inexistente), NÃO invalida o envio: só registra o problema.
    try:
        _executar(
    ...
    with st.form("form_login"):
        ...
        if st.form_submit_button("Entrar", use_container_width=True):
            ...
            if not senha_correta:
                st.error("Senha não configurada nos secrets (SENHA_CORRETOR). Avise o administrador.")
...
# Check-ins reais = quem passou pela fila (exclui lançamentos manuais "Redacall")
...
with aba_fila:
            ...
            with col_salvar:
                if st.button("Salvar e Concluir Atendimento", type="primary", use_container_width=True):
                    if not corretor:
                        ...
                        try:
                            # Trava otimista: só conclui se ainda estiver "Aguardando".
                            ...
                            if not resp.data:
                                st.warning(
                                    "Este aluno já foi concluído ou alterado por outro corretor. "
    ...
    # ==========================================
        ...
        # de auto-refresh: assim a chamada à API (que leva alguns segundos) nunca
        # é interrompida pelo tick do run_every, e a mensagem de status permanece
        # na tela até a próxima interação (não some sozinha).
                ...
                with st.container(border=True):
                    ...
                    with col_acoes:
                        ...
                        if b_chamar.button(rotulo_chamar, key=f"chamar_{aid}", type="primary", use_container_width=True):
                            # Não envia aqui dentro do fragmento (evita corrida com o
        ...
        # Deriva das correções já carregadas em 'todos_dados' (evita query extra)
        if not todos_dados.empty and "status" in todos_dados.columns:
            _conc = todos_dados[todos_dados["status"] == "Concluído"].copy()
            ...
            if st.button("Registrar atendimento", type="primary",
                ...
                if not m_corretor:
                    ...
                    _payload_manual = {
                        ...
                        # Grava a data explicitamente para não depender do default do banco
